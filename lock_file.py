...
class OSLockedAction:
    def __init__(self, target, args = (), kwargs = {}, lockfile = None, timeoutDelay = 7.0, retryPeriod = 0.1, DEBUG = False):
        ...
        >>> time.sleep(5.0)
        Do:
        >>> OSLockedAction(target = time.sleep, args = (5.0,), lockfile="data/myLockFile.lock")
        ...
        try:
            ...
            while 1:
                if timeoutDelay is not None and timeoutDelay != 0.0 and start_deleting_time < time.clock(): # timed out, it must be stale... (i hope)  so remove it.
                    try:
                        if DEBUG: print ("TRYING TO REMOVE...", file=sys.stderr)#print  >>sys.stderr, "TRYING TO REMOVE...",
                        os.remove(lockfile)
                        if DEBUG: print ("REMOVED STALE LOCK...", file=sys.stderr)  #print  >>sys.stderr, "REMOVED STALE LOCK",
                    except OSError:
                        time.sleep(retryPeriod)
                try:
                    if DEBUG: print ("TRYING TO OPEN...", file=sys.stderr)#print  >>sys.stderr, "TRYING TO OPEN...",

                    f = os.open(lockfile,os.O_RDWR | os.O_CREAT | os.O_EXCL)
                    os.write(f,str(__file__) + " made this, called from " + sys.argv[0])

                    if DEBUG: print ("OPENED", file=sys.stderr) #print  >>sys.stderr, "OPENED"
                    break
                except OSError:
                    if DEBUG: print ("SLEEPING...", file=sys.stderr)#print  >>sys.stderr, "SLEEPING..."
                    time.sleep(retryPeriod)
            ...
            # {}
            # was not cleaned up.  Delete it manually if the process is finished.
            # -=-""".format(lockfile), file=sys.stderr)
            ...
            if f:
                if DEBUG:
                    ...
                    print ("REALEASING...", file=sys.stderr)
                os.close(f)
                os.remove(lockfile)
                if DEBUG:
                    # print  >>sys.stderr, "RELEASED"
                    print ("REALEASED...", file=sys.stderr)
...
def example():
    ...
    def action(t, msg):
        ...
        time.sleep(t);
        print (msg)
    ola = OSLockedAction(target = action, args = (10.0,), kwargs = {'msg': 'This is my message!'}, DEBUG=True)
