...
def check_signal_plot(bandpass=False, backgrd_subtract=False):
    ...
    while cycle_id < cycle + 1:
        ...
        print ("row {} col {}".format(str(row), str(column)))
        ...
        # plt.ylabel('amplitude')
        # plt.grid('on')
        # ax1.set_title(battery_id + SoH)
        # ax1.legend(loc='upper right')
        
        # if primary_channel:
        #     x_2 = np.arange(122 * ped, 144 * ped, ped)                           # for gel
        #     avg_2 = avg[122: 144]
        ...
        #     x_3 = np.arange(223*ped, 259*ped, ped)                              # for gel
        #     avg_3 = avg[223 : 260]
        ...
        plt.plot(x_1, avg,label='Echo  v0{:.1f}'.format(float(cycle_id/10)))
        plt.title(' TC28 - Reflection| Caliberation | bandpass [0.2 - 1.0] Mhz | Gain 0.55')
        # plt.xlim((0, 0.00005))
        plt.xlim(0,50)
...
def main ():
    ...
    (2) plot avg of each cycle. Save avg (mean) to csv file
    """
    # check_signal_plot(bandpass = False, backgrd_subtract = False)


    """
    (3) Plot all 64 samplings in a capture. Query data from Mongodb
    """
    echoes_db = database(database='echoes-captures')

    query = {'capture_number':1200}
    projection={
        'raw_data':1
    }
    data = echoes_db.find_one(query=query,projection=projection,
                                     collection="nobatt")
...
address = '/media/kacao/Ultra-Fit/titan-echo-boards/compare/v1-v2-design/'
echoes_index = []
backgrd = []
